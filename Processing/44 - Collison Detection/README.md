...
```java
// Create the size, position, speed and color for a circle
float circleX = 200;    // X position
float circleY = 200;    // Y position
float circleSize = 30;  // Radius of the circle
color circleColor = color(255, 0, 0); // Color of the circle
float xSpeed = 1;       // Speed in the x direction
float ySpeed = 2.5;     // Speed in the y direction
```
...
if (circleX >= width) {  // Right Side
  xSpeed = xSpeed * -1;
} else if (circleX <= 0) { // Left Side
  xSpeed = xSpeed * -1;
}

if (circleY >= height) {  // Top Side
  ySpeed = ySpeed * -1;
} else if (circleY <= 0) { // Bottom Side
  ySpeed = ySpeed * -1;
}
...
```java
if (distFromPoints(circleX, circleY, mouseX, mouseY) < circleSize) {
  // A collision happens!
...
```java
if (distFromPoints(circleX, circleY, mouseX, mouseY) < (circleSize + mouseCircleSize)) {
  // A collision happens!
  circleColor = color(100, 0, 0);
} else {
  // no collision happened
